<?php
    ...
    if(!isset($_SESION['username']) and !isset($_SESSION['password'])){
        ...
        /* answers forum */
        $sql_answ = "SELECT answerspost.text, profiles.username
                        FROM answerspost
                        INNER JOIN profiles
                        ON answerspost.profiles_id = profiles.id
                        WHERE answerspost.posts_id = '$idpost'";
...
<html>
    <head>
        ...
        <link href="css/bigPost.css" rel="stylesheet" media="all">
    </head>
    ...
    <body>
        ...
        <div class="container-fluid" id="mainVegetables"> 
            <?php
                ...
                $sql = "SELECT profiles.username AS username, profiles.name as name, posts.id, posts.texto, posts.image FROM posts INNER JOIN profiles ON posts.profiles_id = profiles.id WHERE posts.id = '$idpost'";
            ...
            <div class="row pub">
                ...
                <div class="col-xs-12 col-md-11">
                    <img src="imgs/postImages/<?php echo $post['image']; ?>" class="picPostExp">
                    <p><?php echo $post['texto'] ?></p>
                </div>
            </div>
            <?php endforeach; ?>
            <a href="postAn.php?i=<?=$row['id']?>" class="button btnGG btnPurple hvr-trim">Responder</a>
            <h3>Comentários:</h3>
