<?php
    ...
    if(!isset($_SESION['username']) and !isset($_SESSION['password'])){
        ...
        if($_SESSION['h'] == 1){
			...
			$sql_tipscat = "SELECT tipscategories.id, tipscategories.category as category
							FROM tipscategories
							ORDER BY tipscategories.id ASC";
			$result_sql_tipscat = mysqli_query($mysqli,$sql_tipscat);
        }else{
            session_destroy();
            unset($_SESSION['username']);
            unset($_SESSION['password']);
...
<html>
    ...
    <body>
        ...
        <div class="container-fluid">
            ...
            <form class="form-horizontal" method="post" action="adminTipsRegister.php">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label for="inputCategory" class="col-md-2 col-form-label">Escolha a categoria da dica</label>
                        <select class="col-md-2" id="inputCategory" name="inputCategory">
							<?php while ($rows_tipscat = mysqli_fetch_assoc($result_sql_tipscat)){ ?>
								<option value="<?=$rows_tipscat['id']?>"><?=$rows_tipscat['category']?></option>
							<?php } ?>
							<!--
                            <option value="1">Luminosidade</option>
                            <option value="2">Umidade do ar</option>
                            <option value="4">Umidade do solo</option>
                            <option value="5">Temperatura</option>
							-->
                        </select>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label for="inputDescription" class="col-md-2 col-form-label">Descreva a dica</label>
                        <div class="col-md-8">
                            <textarea rows="5" id="inputDescription" name="inputDescription"></textarea>
                        </div>
                    </div>
                </div>
                <div class="form-row" id="rowBtn">
                    <button class="button btnGG btnGray hvr-trim" type="reset">Cancelar</button>
                    <button class="button btnGG btnPurple hvr-trim">Registrar</button>
                </div>
            </form>
        </div>
