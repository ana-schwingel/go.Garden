<?php
    ...
    if(!isset($_SESION['username']) and !isset($_SESSION['password'])){
        ...
        // Selecionar todas as categorias de dicas
        $sql_tipscategories = "SELECT id, category FROM tipscategories";
        $result_tipscategories = mysqli_query($mysqli, $sql_tipscategories);
        }else{
            session_destroy();
            unset($_SESSION['username']);
            unset($_SESSION['password']);
    ...
    <body>
        ...
        <div class="container-fluid">
            <?php if(isset($returnDel)){?>
                <h2><?=$returnDel?></h2>
            <?php } ?>
            <h2>Visualização de categorias de dicas</h2>
            <?php while ($rows = mysqli_fetch_assoc($result_tipscategories)){ ?>
            <div class="row">
                <div class="col-md-2">
                    <p><?php echo $rows['category'] ?></p>
                </div>
                <div class="col-md-1">
                    <a href="adminTipsCatEd.php?i=<?=$rows['id']?>" class="button btnGG btnGreen hvr-trim">Editar</a>
                </div>
                <div class="col-md-1">
                    <a href="deleteAdminTipsCat.php?code=<?=$rows['id']?>" class="close">&times;</a>
                </div>
            </div>
            <?php } ?>
        </div>
