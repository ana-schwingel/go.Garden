<?php
    ...
    if(!isset($_SESION['username']) and !isset($_SESSION['password'])){
        ...
        if($_SESSION['h'] == 1){
			...
			$sql_vegetables = "SELECT vegetables.id as id, vegetables.name as name
					FROM vegetables
					ORDER BY vegetables.id ASC";
			$result_sql_vegetables = mysqli_query($mysqli,$sql_vegetables);
        }else{
            session_destroy();
            unset($_SESSION['username']);
            unset($_SESSION['password']);
...
<html>
    ...
    <body>
        ...
        <div class="container-fluid">
            ...
            <h2>Cadastro de dados para o controle</h2>
            <form class="form-horizontal" method="post" action="adminVegControlRegister.php">
                <div class="form-row">
                    <div class="form-group">
                        <label for="inputVegetable" class="col-md-2 col-form-label">Escolha a hortaliça</label>
                        <select class="col-md-2" id="inputVegetable" name="inputVegetable">
							<?php while ($rows_vegetables = mysqli_fetch_assoc($result_sql_vegetables)){ ?>
								<option value="<?=$rows_vegetables['id']?>"><?=$rows_vegetables['name']?></option>
							<?php } ?>
						<!--
                            <option value="001">Alface</option>
                            <option value="002">Tomate</option>
                            <option value="003">Cenoura</option>
						-->
                        </select>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label class="col-md-2" for="inputMinMaxBrightness">Luminosidade</label>
                        <div class="col-md-2">
                            <input type="number" class="form-control" name="inputMinBrightness" id="inputMinBrightness" placeholder="Mínimo">
                        </div>
                        <div class="col-md-2">
                            <input type="number" class="form-control" name="inputMaxBrightness" id="inputMaxBrightness" placeholder="Máximo">
                        </div>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label class="col-md-2" for="inputMinMaxTemperature">Temperatura</label>
                        <div class="col-md-2">
                            <input type="number" class="form-control" name="inputMinTemperature" id="inputMinTemperature" placeholder="Mínimo">
                        </div>
                        <div class="col-md-2">
                            <input type="number" class="form-control" name="inputMaxTemperature" id="inputMaxTemperature" placeholder="Máximo">
                        </div>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label class="col-md-2" for="inputMinMaxSoilDamp">Umidade do solo</label>
                        <div class="col-md-2">
                            <input type="number" class="form-control" name="inputMinSoilDamp" id="inputMinSoilDamp" placeholder="Mínimo">
                        </div>
                        <div class="col-md-2">
                            <input type="number" class="form-control" name="inputMaxSoilDamp" id="inputMaxSoilDamp" placeholder="Máximo">
                        </div>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label class="col-md-2" for="inputMinMaxTemperature">Umidade do ar</label>
                        <div class="col-md-2">
                            <input type="number" class="form-control" name="inputMinAirDamp" id="inputMinAirDamp" placeholder="Mínimo">
                        </div>
                        <div class="col-md-2">
                            <input type="number" class="form-control" name="inputMaxAirDamp" id="inputMaxAirDamp" placeholder="Máximo">
                        </div>
                    </div>
                </div>
                <div class="form-row" id="rowBtn">
                    <button class="button btnGG btnGray hvr-trim" type="reset">Cancelar</button>
                    <button class="button btnGG btnPurple hvr-trim">Cadastrar</button>
                </div>
            </form>
        </div>
