<?php
    ...
    if(!isset($_SESION['username']) and !isset($_SESSION['password'])){
        ...
        $sql_adminTips = "SELECT admtips.text as text, tipscategories.category as tipName, admtips.id as id
							FROM admtips
							INNER JOIN tipscategories
							ON admtips.tipscategories_id = tipscategories.id";
        $result_adminTips = mysqli_query($mysqli, $sql_adminTips);
        }else{
            session_destroy();
            unset($_SESSION['username']);
            unset($_SESSION['password']);
...
<html>
    ...
    <body>
        ...
        <div class="container-fluid">
            <?php if(isset($returnDel)){?>
                <h2><?=$returnDel?></h2>
            <?php } ?>
            <h2>Visualização de dicas</h2>
            <?php while ($rows = mysqli_fetch_assoc($result_adminTips)){ ?>
            <div class="row">
                <div class="col-md-1">
                    <p><strong><?php echo $rows['tipName'] ?></strong></p>
                </div>
                <div class="col-md-9">
                    <p><?php echo $rows['text'] ?></p>
                </div>
                <div class="col-md-1">
                    <a href="adminTipsEd.php?i=<?=$rows['id']?>" class="button btnGG btnGreen hvr-trim">Editar</a>
                </div>
                <div class="col-md-1">
                    <a href="deleteAdminTips.php?code=<?=$rows['id']?>" class="close">&times;</a>
                </div>
            </div>
            <?php } ?>
        </div>
