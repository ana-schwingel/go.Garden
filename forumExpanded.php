<?php
    ...
    $idpost = $_GET['i'];
/*
    $sql = "select * from postsforum where id = '$idpost'";
    $r = mysqli_query($mysqli,$sql);
    $row = mysqli_fetch_assoc($r);
*/

    $sql_pf = "SELECT postsforum.title as title, postsforum.text as text, postsforum.id as id, profiles.name as name, profiles.username as username
                FROM postsforum
                INNER JOIN profiles
                ON postsforum.profiles_id = profiles.id
                WHERE postsforum.vegetables_id = 002
                AND postsforum.id = '$idpost'";
    $r = mysqli_query($mysqli, $sql_pf);
    $row = mysqli_fetch_assoc($r);

    /* answers forum */
    $sql_answ = "SELECT answersforum.text as text, profiles.username as username
                FROM answersforum
                INNER JOIN profiles
                ON answersforum.profiles_id = profiles.id
                WHERE answersforum.postsForum_id = '$idpost'";
    $result_answ = mysqli_query($mysqli,$sql_answ);
?>
<!DOCTYPE html>
<html>
    <head>
        ...
        <link href="css/forumExpanded.css" rel="stylesheet" media="all">
    ...
    <body>
        ...
        <!-- content -->
        <div class="container-fluid" id="mainVegetables"> 
            <div class="row pub">
                <div class="col-xs-12 col-md-1 midTxt">
            ...
            <a href="forumAn.php?i=<?=$row['id']?>" class="button btnGG btnPurple hvr-trim">Responder</a>
            <h3>Veja as soluções compartilhadas:</h3>
            <?php while ($rows = mysqli_fetch_assoc($result_answ)){ ?>
            <div class="row pub">
                <div class="col-xs-12 col-md-1 midTxt">
                    <p><strong><?=$rows['username']?></strong></p>
                </div>
                <div class="col-xs-12 col-md-11">
                    <p><?=$rows['text']?></p>
                </div>
