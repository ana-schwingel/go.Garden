<?php
    ...
    if(!isset($_SESION['username']) and !isset($_SESSION['password'])){
        ...
        $sql_pf = "SELECT postsforum.title as title, postsforum.text as text, postsforum.id as id, profiles.username as username, vegetables.name as name
                    FROM postsforum
                    INNER JOIN profiles
                    ON postsforum.profiles_id = profiles.id
					INNER JOIN vegetables
					ON postsforum.vegetables_id = vegetables.id
                    WHERE postsforum.vegetables_id = '$i'";
        $result_pf = mysqli_query($mysqli, $sql_pf);
		$row = mysqli_fetch_assoc($result_pf);
    }
?>
<!DOCTYPE html>
<html>
    <head>
        ...
        <link href="css/forum.css" rel="stylesheet" media="all">
    </head>
        ...
        <div class="container-fluid" id="mainVegetables">
            ...
            <div class="row" id="rowBtn">
                <div class="col-md-offset-10">
                    <a href="forumPost.php" class="button btnGG btnPurple hvr-trim">Perguntar</a>
                </div>
            </div>
            <?php while ($rows = mysqli_fetch_assoc($result_pf)) { ?>
            <div class="row pub">
                ...
                <div class="col-xs-12 col-md-1 midTxt">
                    <a href="forumExpanded.php?i=<?=$rows['id']?>"><span class="glyphicon glyphicon-plus spnGreen"></span></a>
                </div>
            </div>
            <?php } ?>    
        </div>
