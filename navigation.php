...
<html>
    ...
    <body>
        ...
        <!-- content -->
        <div class="container-fluid">
            <?php if(isset($returnFollow)){?>
                <h2><?=$returnFollow?></h2>
            <?php } ?>
            
            <div class="row" id="rowBtn">
                <div class="col-md-offset-10">
                    <a href="navigationP.php" class="button btnGG btnPurple hvr-trim">Publicar</a>
                </div>
            </div>
            
            <?php
                $myUserId = $_SESSION['userId'];
                $conn = new PDO("mysql:host=127.0.0.1;dbname=new_db_gogarden;charset=utf8", "root", "");
                $sql = "SELECT profiles.username AS username, posts.id as id, posts.texto as texto, posts.image as image
                        FROM posts 
                        INNER JOIN profiles ON posts.profiles_id = profiles.id
                        INNER JOIN follows ON posts.profiles_id = follows.whoFollowId
                        WHERE follows.whoFollowId = posts.profiles_id
                        AND follows.profiles_id = '$myUserId'
